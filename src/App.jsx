...
he:[
  ...
  {type:"q",q:"כמה ספרים יש בתנ״ך?",a:"24",o:["24","39","22","27"],e:"📖"},
...
const ANIMALS_EN = [
  ...
  {emoji:"🐍",name:"Snake",opts:["Snake","Lizard","Turtle","Crocodile"]},
];
const NUMBERS=[
...
function SpeedGame({ t, lang, onBack }) {
  ...
  return(<div className="screen" style={{direction:t.dir}}><div className="topbar"><button className="back-btn" onClick={onBack}>{t.back}</button><div className="score-pill">⭐ {score}</div></div><h2 className="st">⚡ {t.speed}</h2><p className="pl">{round+1}/{total}</p><div className="speed-area"><div style={{display:"flex",gap:16,fontSize:46,justifyContent:"center"}}>{phase==="img1"&&cur.a.map((e,i)=><span key={i}>{e}</span>)}{phase==="img2"&&cur.b.map((e,i)=><span key={i}>{e}</span>)}{(phase==="before"||phase==="ans")&&<span style={{color:"#DDD",fontSize:32}}>• • • •</span>}</div></div><p style={{textAlign:"center",fontSize:16,fontWeight:700,color:"#8B7E74",marginBottom:12}}>{phase==="before"?" ":phase==="img1"?(lang==="he"?"זכור...":"Remember..."):phase==="img2"?(lang==="he"?"מה השתנה?":"What changed?"):t.whatChanged}</p>{phase==="ans"&&cur.opts.map(opt=>{let cls=`opt${isRtl?"":" opt-ltr"}`;if(chosen===opt)cls+=opt===cur.ans?" opt-correct":" opt-wrong";else if(chosen&&opt===cur.ans)cls+=" opt-reveal";return <button key={opt} className={cls} onClick={()=>handle(opt)}>{opt}</button>;})}</div>);
}
